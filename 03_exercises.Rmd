...
>The parameters vegetable and variety are not unique identifiers, so we get many more rows after the mutating join. Certain varieties were planted in different plots, but there was only one harvest recorded for each variety, meaning that the total harvest by variety would overestimate the amount.
...
We can fix this by defining another varible that might identify each indivudal observation better.
...
> (1) Group each dataset by vegetable. (2) Create a new varible to summarize the total cost of planting each vegetable. (3) use left_join?? (4) Multiply the in-store price by the number of lbs harvested (taken from garden harvest) to get the total price of buying each vegetable type. (5) Compute the difference between the total cost of buying the produce and the total cost of buying the seeds for each one tp get the money saved by planting seeds.
...
>The pattern observed is that the density (amount of bike rentals) is much higher in the hours in which people are going and coming to work during the weekdays. On the weekends, the density is higher during times when people 
...
```
  ...
  >I like this display better because you can clearly differentiate the patterns between registered and casual clients. However, I think it could also be confusing to interpret because one might think that the upper color only encompases the area of that color and not the total colored area below it.
...
```
  ...
  >It tell us how the client and non-client rentals fluctuate throughout the week in separate plots. It also does not group by weekend date, so we can compare the two plots more easily.
...
breed_traits_total %>% 
  ...
  mutate(Breed_upper = str_to_upper(Breed)) %>% 
  ggplot(aes(x = rank_total, y = desirable_traits))+
  geom_point(aes(color = Breed_upper))
